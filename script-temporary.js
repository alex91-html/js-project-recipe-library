...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const isRandom = document.getElementById('filter-random')?.checked; // check what is this? 
  ...
  const loadRecipes = (recipes) => {
    ...
    recipes.forEach(recipe => {
      const ingredientsList = recipe.ingredients.map(ingredient => `<li>${ingredient}</li>`).join(''); // better understand – recipe.ingredients.map – 
